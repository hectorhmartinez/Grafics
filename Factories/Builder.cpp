...
#include"Factories/SceneFactoryVirtual.h"
...
void Builder::newVirtualScene() {
    ...
    QString filename = QFileDialog::getOpenFileName();
    if(!filename.isNull()){
        shared_ptr<SceneFactoryVirtual> vir = make_shared<SceneFactoryVirtual>();
        scene = vir->createScene(Serializable::Json,filename);
        emit newScene(scene);
    }
...
void Builder::newDataScene()
{
    ...
    if(!name.isNull()){
        //shared_ptr<SceneFactoryData>  data = make_shared<SceneFactoryData>();
        //scene = data->createScene(Serializable::Json,name);
        emit newScene(scene);
    }
